...
var metaurl = `${typesenseHost}/collections/${collectionName}/documents`

exports.search = function (query, options) {
    let url = `${metaurl}/search?q=${query}&${options}`
...
var invokeIndexer = function() {
    let listExistedAjsonlnFile = fs.readdirSync('/tmp').filter(fn => fn.endsWith('.ajsonln'));
    listExistedAjsonlnFile.forEach(ajsonlnFile => indexDoc('/tmp/' + ajsonlnFile))
